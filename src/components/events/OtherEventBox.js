...
const OtherEventBox = ({ eventData, general }) => {
  return (
    <View style={styles.boxStyle} key={eventData.relationId}>
      <View style={styles.header}>
        <View style={{ flexDirection: "row" }}>
          <Image source={ICONS.other} style={{ width: 20, height: 20 }} />
          <Text style={[styles.titleTextFont, styles.titleHeader]}>
            {LABELS.setLabel("OtherEvent")}
          </Text>
        </View>
        <EventMenu
          showCopy={false}
...
const styles = StyleSheet.create({
  header: {
    ...
    padding: 10,
    backgroundColor: COLORS.otherEventBox,
    marginBottom: 10,
    flexDirection: "row",
